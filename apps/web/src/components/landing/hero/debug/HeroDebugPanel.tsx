...
const GROUPS: readonly { title: string; fields: readonly Field[] }[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'starSizeMax', label: 'sizeMax (px)', min: 0.4, max: 12, step: 0.05 },
      { key: 'starTwinkle', label: 'twinkleAmount', min: 0, max: 0.4, step: 0.005 },
      { key: 'starHighlight', label: 'highlight halo', min: 0, max: 1.5, step: 0.01 },
...
function configSnippet(t: HeroTuning): string {
  ...
  return [
    ...
    `stars: { countDesktop: ${n(t.starCount)}, driftSpeed: ${n(t.driftSpeed)}, slideSpeed: ${n(t.slideSpeed)}, sizeRange: [${n(t.starSizeMin)}, ${n(t.starSizeMax)}], twinkleAmount: ${n(t.starTwinkle)}, highlightHalo: ${n(t.starHighlight)}, fogDensity: ${n(t.starFogDensity)} }`,
